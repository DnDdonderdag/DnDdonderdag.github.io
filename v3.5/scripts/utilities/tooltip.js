...
function showTooltip(event) {
  if (event.keyCode == '18' && document.activeElement != document.body) {
    ...
    if (activeElement.className.includes('sync')) {
      ...
      let preSliced = classname.slice(
        activeElement.className.indexOf('syncMe')
      ); // filtering the syncclass from within the enitre class string
      let syncClass =
        preSliced.indexOf(' ') == -1
          ? preSliced.slice(6)
          : preSliced.slice(6, preSliced.indexOf(' ')); // finishing filtering (this is a one line if-else)
      tooltip.textContent += '\nSyncClass:' + syncClass;
