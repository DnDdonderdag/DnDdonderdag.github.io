...
export function create(
  ...
  idAddition =
    idAddition === undefined || idAddition === null ? '' : idAddition;

  if (level == 0) {
    let cantripAsset = svg.create(
      'cantripBarAsset' + idAddition,
      top,
      left,
      197,
      50,
    ...
    if (lineBool) {
      for (let j = 0; j < lineAmount; j++) {
        let spellLine = sync.syncDecorator(
          'spellLine' + String(j) + 'Level' + level,
          calc.calcDecorator(
            formfield.singleLine(
              'spellLine' + String(j) + 'Level' + level + idAddition,
              45 + 12 * j,
              12,
              180,
              11,
        ...
        line.id =
          'spellLine' + String(j) + 'Level' + level + 'Asset' + idAddition;
    ...
    let SpellBarAsset = svg.create(
      'level' + level + 'SpellBarAsset' + idAddition,
      top,
      left,
      197,
      50,
    ...
    for (let i = 0; i < 4; i++) {
      let spellLineCheckmark = sync.syncDecorator(
        'spellCheckmark' + String(i) + 'Level' + level,
        button.checkmark(
          'level' + level + 'SpellCheckMark' + String(i) + idAddition,
          16,
          90 + i * 24,
          18,
          18,
      ...
      if (idAddition == 'spellListPage') {
        spellLineCheckmark.style.setProperty(
          'transform',
          'scale(1,calc(1/1.095))'
        );
      }
    }
    sync.syncDecorator(
      'spellAmountLevel' + level,
      formfield.singleLine(
        'spellAmountLevel' + level + idAddition,
        14,
        23,
        45,
        22,
    ...
    if (lineBool) {
      for (let j = 0; j < lineAmount; j++) {
        ...
        let spellLineButton = button.tickbox(
          'button' + String(j) + 'Level' + level,
          50 + 12 * j,
          2,
          8,
          8,
        ...
        if (idAddition == 'spellListPage') {
          spellLineButton.style.setProperty(
            'transform',
            'scale(1,calc(1/1.095))'
          );
        }
