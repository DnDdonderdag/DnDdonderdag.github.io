...
export function create(
  ...
  if (formBool) {
    if (labelText) {
      frameFormField = autoSize.autoSizeDecorator(
        formfield.create(
          id,
          7,
          10,
    ...
    if (syncClass) {
      frameFormField = sync.syncDecorator(syncClass, frameFormField);
    }
  } else {
    if (labelText) {
      text.create(
  ...
  const positions = [
    '--top:0px; --left:0px;',
    ' --top:0px; --left:' + String(width - 20) + 'px;',
    ' --top:' +
      String(height - 20) +
      'px; --left:' +
      String(width - 20) +
      'px;',
    ' --top:' + String(height - 20) + 'px; --left:0px;',
  ];
  for (let i = 0; i < 4; i++) {
    ...
    corner.style =
      '--rotate:' + rotations[i] + '; --width:20px; ' + positions[i];
    frame.appendChild(corner);
  }
  //Create sides
  const scales = [
    '--scale:scale(' + String((width - 15) / 20) + ',1);',
    '--scale:scale(' + String((width - 15) / 20) + ',-1);',
    '--scale:scale(1,' + String((height - 38) / 20) + ');',
    '--scale:scale(-1,' + String((height - 38) / 20) + ');',
  ];
  const tops = [
    '--top:0px;',
    ' --top:' + String(height - 20) + 'px;',
    ' --top:' + String(height / 2 - 10) + 'px;',
    ' --top:' + String(height / 2 - 10) + 'px;',
  ];
  const lefts = [
    '--left:' + String((width - 20) / 2) + 'px;',
    '--left:' + String((width - 20) / 2) + 'px;',
    '--left:' + String(width - 20) + 'px;',
    '--left:0px;',
  ];
  for (let i = 0; i < 4; i++) {
